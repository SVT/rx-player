...
export default function BufferOrchestrator(content, clock$, abrManager, sourceBuffersManager, segmentPipelinesManager, options) {
    ...
    var segmentBookkeepers = new WeakMapMemory(function () { return new SegmentBookkeeper(); });
    ...
    var outOfManifest$ = clock$.pipe(mergeMap(function (_a) {
        ...
        if (position < manifest.getMinimumPosition()) {
            ...
            var warning = new MediaError("MEDIA_TIME_AFTER_MANIFEST", "The current " +
    ...
    function manageEveryBuffers(bufferType, basePeriod) {
        ...
        var hasLoadedABuffer = false; // true after the first PeriodBuffer is ready
        /**
         * @param {Object} period
         * @returns {Observable}
         */
        ...
        var restartBuffersWhenOutOfBounds$ = clock$.pipe(filter(function (_a) {
            ...
            if (newInitialPeriod == null) {
                throw new MediaError("MEDIA_TIME_NOT_FOUND", "The wanted position is not found in the Manifest.", true);
            }
            return launchConsecutiveBuffersForPeriod(newInitialPeriod);
        }));
        return observableMerge(launchConsecutiveBuffersForPeriod(basePeriod), restartBuffersWhenOutOfBounds$);
    }
    /**
