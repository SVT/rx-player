 ...
 * @returns {number}
 */
export default function getPresentationLiveGap(parsedMPD, lastTimeReference) {
    if (lastTimeReference != null) {
        var ast = parsedMPD.availabilityStartTime || 0;
        var now = Date.now() - (parsedMPD.clockOffset || 0);
